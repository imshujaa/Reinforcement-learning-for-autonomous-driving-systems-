...
class IntersectionEnvMA(AbstractEnv):
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        ma_reward = [reward,adv_reward]

        return obs, ma_reward, terminated, truncated, info
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        #     self._spawn_vehicle(np.linspace(0, 80, self.config["vehicles_count"])[t])
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            if ego_id == 1:
                # RANDOM CHOICE
                entry_options = [1, 2, 3]  # Avoid 0 (South) (EGO STARTS HERE)
                start = self.np_random.choice(entry_options)
                remaining = [r for r in range(4) if r != start]
                end = self.np_random.choice(remaining)
                route = [start, end]
...
class MultiAgentIntersectionEnv(IntersectionEnvMA):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update(
...
class ContinuousIntersectionEnv(IntersectionEnvMA):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update(
